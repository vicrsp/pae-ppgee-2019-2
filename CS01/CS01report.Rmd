...
```{r plot, echo=FALSE, fig.width=7,fig.height=4, fig.cap="Boxplot e gráfico quantil-quantil das amostras"}
par(mfrow=c(1,2))
boxplot(data.mean.test, horizontal = TRUE, main="Boxplot")
qqPlot(data.mean.test, las = 1, pch = 16, add.line = TRUE, main="Normal Q-Q plot")
...
Mesmo havendo falha na aceitação da hipotese de normalidade dos dados, para testar a hipotese nula para a média utilizou-se o teste t (função `t.teste`). É possivel a utilização deste teste devido normalidade alcançada por uma reamostragem, verificado pelo o teorema do limite central. 
...
O teste t retorna que a hipotese alternativa é verdadeira para uma média maior que 50. Porém, pode-se verifcar que o p-valor é maior do que o nível de significância estabelecido ($\alpha = 0.01$). Desta maneira não há evidência suficiente para se rejeitar a hipótese nula.
...
O valor da media retornado pelo teste t é facilmente verificado dentro do intervalo de confiança calculado, afirmando, desta maneira, a não rejeição da hipotese nula.

### Validação de premissas

Para validação das premissas utilizadas no teste t, realizou-se uma reamostragem dos dados, conforme descrito a seguir.
...
qqPlot(means.re, las = 1, pch = 16, add.line = TRUE)
```

Analisando o gráfico quantil-quantil para a reamostragem, é fácil atestar a normalidade da distribuição da média.
