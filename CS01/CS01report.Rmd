...
(mean.t.teste <- t.test(data.mean.test, 
                  ...
                  conf.level = ci_mean))
```
Como a média amostral é 49.03614 e dado que estamos considerando um teste de cauda superior, a hipótese nula é rejeitada. Portanto, podemos afirmar que não houve ganho de desempenho na média do novo sistema para a significância estabelecida ($\alpha = 0.01$).
...
O valor da média retornado pelo teste t é facilmente verificado dentro do intervalo de confiança calculado, que vai de 47.43 a infinito, confirmando, desta maneira, a rejeição da hipótese nula.
