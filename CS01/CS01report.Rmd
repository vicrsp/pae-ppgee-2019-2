...
(mean.t.teste <- t.test(data.mean.test, 
                  ...
                  alternative = "greater",  #hipótese alternativa
                  conf.level = ci_mean))
```

O teste t retorna que a hipótese alternativa é verdadeira para uma média maior que 50. Porém, pode-se verifcar que o p-valor é maior do que o nível de significância estabelecido ($\alpha = 0.01$). Desta maneira não há evidência suficiente para se rejeitar a hipótese nula.
...
O valor da média retornado pelo teste t é facilmente verificado dentro do intervalo de confiança calculado, que vai de 47.43 a infinito, confirmando, desta maneira, a não rejeição da hipótese nula.
...
```{r média reamostragem}
mean(means.re)
```

Graficamente:

```{r gráfico_reamostragem_normal, fig.width=5,fig.height=4, fig.cap="Gráfico quantil-quantil para reamostragem da média"}
