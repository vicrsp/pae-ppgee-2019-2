...
# Summary
O presente trabalho realizou o delineamento e executou os testes estatísticos para avaliar as diferenças no IMC médio entre duas populações de estudantes de pós-graduação em Engenharia elétrica, nos semetres de 2016-2 e 2017-2. As subpopulações masculina e feminina foram analisadas separadamente. Após os testes verificou-se que as médias da subpopulação masculina são iguais, porém para a feminina elas são diferentes, para um intervalo de confiança de 95%. 
...
Neste trabalho foi feito um estudo estatístico da diferença das médias entre duas turmas de alunos de pós-graduação. Os dados foram divididos entre uma subpopulação masculina e feminina, as quais foram analisadas separadamente. Para a masculina, todas as premissas são atendidas, logo o teste t foi utilizado e p-valor `r mean.t.teste.male$p.value` obtido comprova que não podemos rejeitar a hipótese nula, ou seja, as médias não são diferentes. Já para a subpopulação feminina, a premissa de normalidade não é respeitada para os dados de 2017-2, o que fez com que fosse necessário utilizar técnicas não paramétricas para estimativa do intervalo de confiança. A técnica de bootstrapping foi aplicada à média das diferençase IMC entre as turmas, e obteve-se que a hipótese nula é rejeitada, ou seja, as médias são diferentes.
