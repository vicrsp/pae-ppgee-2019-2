...
Para a análise da significância, utilizou-se a função `pwr.t.test` com o número de amostras $n$ e potência $\pi = 0.8$ especificados. Os resultados dos testes podem ser vistos abaixo.
...
É possível verificar, analisando os resultados, que existem amostras suficientes para a realização dos testes de cada subpopulação, que atendam o nível de siginificancia $\alpha=0.05$ pré estabelicido. É importante notar que, dado que existem mais amostras de estudantes do sexo masculino, a significância mínima estimada foi significativamente inferior a da população feminina. 
...
Conforme esperado, a potência do teste referente a população masculina é `r 100*round((pwr.est.males$power - pwr.est.females$power)/pwr.est.males$power,digits=2)`% maior que a feminina, indicando uma maior probabilidades de erros do tipo II. Assim como no teste da significância as duas amostras possuem potência superior a estimada.
...
Mesmo após o ajuste logarítmico, os dados continuam sem apresentar uma distribuição normal, de acordo com o p-valor obtido no teste de Shaphiro-Wilk.
...
Devido o baixo número de amostras da subpopulação, realizou-se um teste de potência, utilizando a função pwr.t.test, para verificar a probabilidade de erro de tipo II. O teste está especificado a seguir.

``` {r powr_test_fem}
pwr.t2n.test(n1 = 7, n2= 4, d = sig.est.females$d, sig.level = 0.05, power = NULL,
    alternative = "two.sided")
```

O teste resultou em uma potência extremamente baixa devido ao baixo número de amostras entre os dois semestres 2016-2 e 2017-2. Desta maneira a probabilidade de erro de tipo II é extremamente alta. 

Para melhora a análise dos dados e aumentando a sua potências realizou-se uma remostragem utilizando-se a técnica de bootstrap, conforme mostrado a seguir.
...
(pot_males<-pwr.t2n.test(n1 = 21, n2= 21, d = sig.est.males$d, sig.level = 0.05, power = NULL,
    alternative = "two.sided"))
```

O teste resultado em uma potência superior de `r pot_males$power`, que é considerada uma potência próxima do valor pré-estabelecido.
