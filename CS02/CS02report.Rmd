...
O objetivo deste experimento é estudar as diferenças entre o IMC médio entre duas turmas - 2016-2 e 2017-2 - de estudantes de pós-graduação em Engenharia elétrica da UFMG. A análise foi dividada entre as subpopulações de homens e mulheres, visando comparar as subpopulações das duas amostras entre si. Essa separação deve-se às diferenças no IMC entre homens e mulheres, e também no tamanho amostral. 
...
summary(data.2017.2)
```
Analisando os dados é possível observar que a quantidade de amostras do sexo feminimo é inferior à do masculino, o que pode afetar a potência dos testes a serem executados. Para melhor visualização, um gráfico boxplot foi gerado na figura abaixo.
...
Portanto, é obtido um tamanho de efeito de `r sig.est.females$d` para os dados do gênero feminino e `r sig.est.males$d` para o masculino, fixando a potência em 0.8. Pelo teste percebe-se que para a mesma potência os dados do sexo feminino possuem um tamanho de efeito maior, isto é, a diferença percebida na média é maior em relação àquela percebida no teste para o sexo masculino. Esse fato deve-se ao número de amostras femininas ser bem inferior.
...
Para realizar as inferências estatísticas sobre o IMC das duas populações é necessário validar as premissas antes de executar os respectivos testes. Neste caso, como a variância das duas populações é desconhecida, utiliza-se a distribuição t para o teste de hipóteses e para os intervalos de confiança [@montgomery2007applied]. 
...
Diante dos resultados, foi necessário fazer uma transformação logarítmicaa nos dados de 2017-2 visando normalizá-los, e possibilitar a realização dos testes estatísticos.
...
Pela análise do gráfico e do teste de Shapiro Wilk após a transformação logarítmica, os dados de 2017.2 continuam sem apresentar uma distribuição normal, pois o p-valor permanece inferior a 0.05.
...
Pela análise do teste de Shapiro-Wilk e dos gráficos conclui-se que não há evidências para rejeitar a hipótese nula para as duas amostras, pois em ambas o p-valor do teste foi superior a 0.05. Portanto, para a subpopulação masculina dos dois semestres os dados estão normalmente distribuídos.
...
Para o teste de hipótese da subpopulação feminina utilizou-se o teste t, analizando a igualdade das médias amostrais.
...
Devido ao baixo número de amostras da subpopulação, realizou-se uma remostragem utilizando-se a técnica de bootstrap como forma de obter uma melhor aproximação do intervalo de confiança, conforme mostrado a seguir.
...
O teste t para as subpopulações masculinas resultou em um p-valor superior ao nível de significância de $0.05$, como especificado. Deste forma não se possui evidências suficientes para rejeitar a hipótese nula. Verifica-se também que o a valor das diferenças entre as médias está dentro do valor especificado. 
...
Note que neste caso, tanto para a média quanto para a mediana, não hpa evidências para rejeitar a hipótese nula. Isso reforça ainda mais os resultads obtidos para o teste t. 
