...
title: "Comparação de desempenho de turmas em um processo de produção de aço"
...
<img src="steel_making.gif" alt="drawing" class="center-smaller"/>
...
Análise do desempenho das turma de operários em relação ao consumo de oxigênio no processo de produção do aço usando um forno elétrico.
...
* **Blocagem** - Usada para eliminar ruídos conhecidos e controláveis [@Montgomery:2006:applied] - Será usada para retirar a Sazonalidade dos dados em relação aos meses;
...
<img src="Design_Experiment.png" alt="drawing" class="center-smaller"/>


## Planejamento do Experimento {.smaller}
...
Desta forma, aplica-se o testa **ANOVA** utilizando um modelo linear [@montgomery2007applied].

## Planejamento do Experimento | Modelo {.smaller}
...
$(\tau\beta)_{ij}$ : efeito da interação da turma $i$ com o material $j$;

$\delta_k$ : efeito da blocagem do mês;

$\epsilon_{ijk}$ : resíduos.
...
$$ \frac{F_{(1- \alpha)}^{a-1}}{F_{\beta, \phi}^{ab(n-1)}} = 1, \quad  \phi = \frac{bn\sum_{i=1}^{a}\tau_{i}^2}{a\sigma^2}$$
...
* O modelo possui um $R^2$ muito baixo: os fatores turma e material sozinhos conseguem explicar 6% da variância dos dados. Mesmo assim, os resultados do ANOVA não deixam de ser significativos, já que não estamos interessados na predição do consumo [@HowtoInt53:online].
