...
Número de amostras para os fatores considerados.
```{r heat_count, echo=FALSE, fig.width=8,fig.height=5, fig.align='center'}

ggplot(data.materials, aes(x=mes, y=VarConsumo_Eletrica)) + 
...
anv.quim <- aov((VarConsumo_Quimica) ~  turno * material + mes, data = data.materials)
anv.ener <- aov((VarConsumo_Eletrica) ~  turno * material + mes, data = data.materials)
anv.temp <- aov((VarTempoProcesso) ~  turno * material + mes, data = data.materials)
...
```{r multicomparison1}
tk_test_quim <-  glht(anv.quim, linfct = mcp(material = "Tukey"))
plot(tk_test_quim)
```
...
```{r multicomparison2}
tk_test_ener <-  glht(anv.ener, linfct = mcp(material = "Tukey"))
plot(tk_test_ener)
```
...
```{r multicomparison2}
tk_test_temp <-  glht(anv.temp, linfct = mcp(material = "Tukey"))
plot(tk_test_temp)
```
