...
Para o seguinte experimento foram realizadas as seguintes etapas: 
...
Para a análise comparativa entre as configurações do algortimo DE, determinou-se as seguintes hipóteses a serem testadas.
...
Z <- read.csv(fname)
Z[,"result"] = -1
kable(Z[1:5, 3:6], caption = 'Exemplo de arquivo de configuração', align = 'l')
```
...
```{r read_data}
sample.all <- read.csv('rcbd.config.maressa3.csv', header = TRUE)
sample.all$configuration <- as.factor(sample.all$algorithm)
sample.all <- sample.all %>% mutate(logresult = log(result))

sample.all.eda <- sample.all %>% filter(instance == 18 | instance == 70 | instance == 130)
```
...
De acordo com Campelo [@Campelo2018], o número de blocos ideal é calculado variando a quantidade de blocos enquanto a relação
...
Para realizar as inferências estatísticas sobre as duas configurações do algoritmo de otimização é necessário validar as premissas antes de executar o teste. Neste caso, como tratam-se de duas configurações em um espectro amplo de dimensões, existe um fator conhecido e controlável que pode influenciar no resultado do teste. Então, para eliminar o efeito desse fator indesejável uma opção é realizar a blocagem [@montgomery2007applied]. A seguir são apresentados os testes realizados para validar as premissas exigidas pelo teste.

A - Normalidade

Para se ter uma ideia inicial da normalidade dos dados, o histograma para as instâncias em avaliação foi gerado (Figura \ref{fig:histogram}).
...
Pelo histograma apresentado, é possível notar que os dados não apresentam uma distribuição visivelmente normal. Os gráficos quantil-quantil e os testes de Shapiro-Wilk são apresentados para comprovar essa observação.

```{r plot_norm_dim4, echo=FALSE, fig.width=7,fig.height=7, fig.cap="Gráfico quantil-quantil das instâncias avaliadas"}
sample.all.dim.4.algo1 <- sample.all.eda %>% filter(algorithm == 1 & instance  == 18)
sample.all.dim.4.algo2 <-  sample.all.eda %>% filter(algorithm == 2 & instance == 18)
sample.all.dim.50.algo1 <- sample.all.eda %>% filter(algorithm == 1 & instance == 70)
sample.all.dim.50.algo2 <- sample.all.eda %>% filter(algorithm == 2 & instance == 70)
sample.all.dim.100.algo1 <- sample.all.eda %>% filter(algorithm == 1 & instance == 130)
sample.all.dim.100.algo2 <- sample.all.eda %>% filter(algorithm == 2 & instance == 130)
...
sw.test.result[1,] <- c(shapiro.test(sample.all.dim.4.algo1$result)$p.value,1,10)
sw.test.result[2,] <- c(shapiro.test(sample.all.dim.4.algo2$result)$p.value,2,10)
sw.test.result[3,] <- c(shapiro.test(sample.all.dim.50.algo1$result)$p.value,1,70)
sw.test.result[4,] <- c(shapiro.test(sample.all.dim.50.algo2$result)$p.value,2,70)
sw.test.result[5,] <- c(shapiro.test(sample.all.dim.100.algo1$result)$p.value,1,130)
sw.test.result[6,] <- c(shapiro.test(sample.all.dim.100.algo2$result)$p.value,2,130)
...
sw.test.result.log[1,] <- c(shapiro.test(sample.all.dim.4.algo1$logresult)$p.value,1,18)
sw.test.result.log[2,] <- c(shapiro.test(sample.all.dim.4.algo2$logresult)$p.value,2,18)
sw.test.result.log[3,] <- c(shapiro.test(sample.all.dim.50.algo1$logresult)$p.value,1,70)
sw.test.result.log[4,] <- c(shapiro.test(sample.all.dim.50.algo2$logresult)$p.value,2,70)
sw.test.result.log[5,] <- c(shapiro.test(sample.all.dim.100.algo1$logresult)$p.value,1,130)
sw.test.result.log[6,] <- c(shapiro.test(sample.all.dim.100.algo2$logresult)$p.value,2,130)
...
Pelas tabelas 2 e 3, é possível notar que após a aplicação da transformação logarítmica, há resultados que deixam de ser normais. Portanto, não podemos afirmar que para todas as dimensões e configurações testadas os dados seguem uma distribuição normal.
