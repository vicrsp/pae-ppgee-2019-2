...
O teste paramétrico usado para avaliar a blocagem é a ANOVA.
...
```
De acordo com o resultado do teste existem evidências de que a média dos dois algoritmos não são iguais. Além disso, o modelo obteve um ajuste satisfatório com o $r^2 = 0.974$, isto é, o modelo explica mais de 97% dos resultados dos dois algoritmos.

```{r plot_norm_dim4, echo=FALSE, fig.width=7,fig.height=7, fig.cap="Gráfico de teste das premissas\\label{fig:norm}"}
par(mfrow = c(2, 2))
plot(model, pch = 20, las = 1)

```
NÃO SEI ANALISAR ESSE GRÁFICOS...
...
Pela análise dos resultados, pode-se concluir que o modelo também ajusta bem aos dados, porém o modelo anterior obteve melhor ajuste.
...
a           <- length(unique(data.block.instances))
b           <- length(unique(data.block.instances$instance))
((b - 1) * MSblocks + b * (a - 1) * MSe) / ((a * b - 1) * MSe)
```
O QUE SIGNIFICA ESSA EFICIÊNCIA DA BLOCAGEM??

```{r kruskal, echo = FALSE}
multcomp1 <- kruskal.test(result~algorithm,data=data.block.instances)
multcomp1
```

```{r DuntestCI, echo = FALSE}
...
Pela análise dos resultados conclui-se que os dois algoritmos apresentam desempenhos diferentes, pois o intervalo de confiança não contém o valor zero (alvo). Neste caso, a configuração 2 obteve resultados melhores.

## Teste de Hipótese - Não Paramétrico

Verificando que os dados gerados pelas duas configurações não são normais e que não há um homegeniedade entre as variâncias, utilizou-se também um teste não paramétrico. O teste utilizado para verificar a igualdade das médias entre as duas configurações foi o teste de Friedman. O teste de Friedman é um teste não paramétrico que generaliza o teste de sinais e possui um poder estatítico modesto para muitas distribuições não normais [@zimmerman1993relative]. O teste Friedman está descrito abaixo.
