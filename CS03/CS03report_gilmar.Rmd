...
#means_group <- data.frame("Group" = rep(0,G), "mean" =  rep(0,G), "algorithm" = rep(0,G))
data_c <- data.frame("value" = rep(0,6*G))
k <- 1
for (i in 1:G){
  ...
  data_c[k] <- i
  data_c[k+1] <- mean(block_i1$result)
  data_c[k+2] <- i
  data_c[k+3] <- mean(block_i2$result)
  
  k <- k + 4
}

datas <- matrix(data_c$value, nrow = 2)
