...
Portanto, o número mínimo de blocos necessários $b$ é de `r min((tb %>% filter(ratio <= 1 & ratio > 0))$b)`. As iterações podem ser vistas na tabela abaixo.
...
data.block.instances.boot <- boot(data=data.block.instances, statistic=bs,
   R=5000, formula=resultLog  ~ algorithm + instance)

boot.ci(data.block.instances.boot, type="bca", conf = 0.975)
```
