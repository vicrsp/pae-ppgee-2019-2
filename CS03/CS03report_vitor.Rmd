...
if(!file.exists('CS03data_ea.csv')){
  ...
  write.csv('CS03data_ea.csv',x = sample.all, row.names = FALSE)

  } else{
  sample.all <- read.csv('CS03data_ea.csv')
  sample.config1 <- sample.all[which(sample.all$config == 'config1'),]
  sample.config2 <- sample.all[which(sample.all$config == 'config2'),]
}
...
ggplot(sample.all, aes(x=config, y=value)) + geom_boxplot()
```

```{r plot_histogram, echo=FALSE, fig.width=7,fig.height=4, fig.cap="Boxplot dos dados\\label{fig:boxplot}"}
ggplot(sample.all, aes(x=value, fill=config)) + geom_histogram(bins = 50)
```
